...
def get_camera_intrinsics(camera_component):
    if camera_component is None:
        ...
        return {
            ...
            "height": 1080,
            "sensor_width": 24.576,
            "sensor_height": 13.824,
            "focal_length": 35.0,
        }
    
    intrinsics = {}
    
    # Read from Filmback settings (sensor dimensions in mm)
    try:
        filmback = camera_component.get_editor_property("filmback")
        if filmback:
            sensor_width = filmback.get_editor_property("sensor_width")
            sensor_height = filmback.get_editor_property("sensor_height")
            intrinsics["sensor_width"] = float(sensor_width) if sensor_width is not None else 24.576
            intrinsics["sensor_height"] = float(sensor_height) if sensor_height is not None else 13.824
        else:
            intrinsics["sensor_width"] = 24.576
            intrinsics["sensor_height"] = 13.824
    except Exception as e:
        logger.warning(f"Failed to read filmback settings: {e}")
        intrinsics["sensor_width"] = 24.576
        intrinsics["sensor_height"] = 13.824
    
    # Read Current Focal Length (mm)
    try:
        focal_length = camera_component.get_editor_property("current_focal_length")
        intrinsics["focal_length"] = float(focal_length) if focal_length is not None else 35.0
    except Exception as e:
        logger.warning(f"Failed to read current_focal_length: {e}")
        intrinsics["focal_length"] = 35.0
    
    # Read Current Horizontal FOV (degrees)
    try:
        fov = camera_component.get_editor_property("current_horizontal_fov")
        intrinsics["fov"] = float(fov) if fov is not None else 90.0
    except Exception as e:
        logger.warning(f"Failed to read current_horizontal_fov: {e}")
        intrinsics["fov"] = 90.0
...
def export_camera_data(sequence_path, binding_name_camera, output_dir, actor_blueprint_class_path=None):
    ...
    with open(output_intrinsic_csv, 'w', newline='') as f_intr:
        ...
        writer_intr.writerow(["fov", "aspect_ratio", "width", "height", "sensor_width", "sensor_height", "focal_length"])
