...
class MockBosManager(BosManager):
    """模拟BOS管理器（用于测试）"""
    
    def __init__(self, existing_scenes: List[str] = None):
        """
        Args:
            existing_scenes: 模拟存在的场景列表
        """
        super().__init__()
    ...
    def check_scene_exists(self, bucket: str, scene_path: str) -> bool:
        """模拟检查场景存在性"""
        # 从路径提取场景名
        scene_name = scene_path.rstrip('/').split('/')[-1]
        return scene_name in self.existing_scenes
    
    def download_scene(self, bucket: str, scene_path: str, local_path: Path) -> bool:
        """模拟下载"""
        print(f"[模拟] 下载场景: {scene_path} -> {local_path}")
        local_path.mkdir(parents=True, exist_ok=True)
        return True
    
    def upload_scene(self, bucket: str, local_path: Path, bos_path: str) -> bool:
        """模拟上传"""
        print(f"[模拟] 上传场景: {local_path} -> {bos_path}")
        return True
