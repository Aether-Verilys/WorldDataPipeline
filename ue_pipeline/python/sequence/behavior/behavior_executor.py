...
from ..navigation.nav_utils import (
    distance_cm,
    find_path_points,
    project_to_nav,
...
try:
    from ..navigation.navmesh_connectivity import get_spawn_point_with_connectivity
...
def _smooth_yaw_transition(
    ...
    """
    Generate smooth yaw transition from start to target with rotation speed limit.
    
    Args:
        start_yaw: Starting yaw angle in degrees
        target_yaw: Target yaw angle in degrees
        num_steps: Number of interpolation steps (frames)
        max_yaw_rate_deg_per_sec: Maximum rotation speed in degrees/second
        fps: Frames per second
    
    Returns:
        List of yaw values with smooth transition
    """
    if num_steps <= 1:
        return [start_yaw]
    ...
    start_yaw = normalize_angle(start_yaw)
    target_yaw = normalize_angle(target_yaw)
    
    # Calculate shortest angular distance
    delta = target_yaw - start_yaw
    if delta > 180:
        delta -= 360
    elif delta < -180:
        delta += 360
    
    # Calculate maximum rotation per frame
    max_rotation_per_frame = max_yaw_rate_deg_per_sec / fps
    ...
    for i in range(num_steps):
        ...
        if i < num_steps - 1:
            # Calculate angle to target (shortest path)
            angle_to_target = target_yaw - current_yaw
            # Normalize to [-180, 180] to ensure shortest rotation
            ...
            if abs(angle_to_target) <= max_rotation_per_frame:
                # Can reach target in next step
                rotation = angle_to_target
            else:
                # Rotate at maximum speed towards target
                rotation = max_rotation_per_frame if angle_to_target > 0 else -max_rotation_per_frame
            
            current_yaw = normalize_angle(current_yaw + rotation)
    
    # Final pass: ensure all yaw values are normalized to [-180, 180]
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if path_points and len(path_points) >= 2:
                ...
                # Calculate target yaw at the end (based on final trajectory direction)
                if len(path_points) >= 2:
                    dx = path_points[-1].x - path_points[-2].x
                    dy = path_points[-1].y - path_points[-2].y
                    target_yaw = math.degrees(math.atan2(dy, dx))
                else:
                    target_yaw = context.yaw
                
                # Get max yaw rate from config (default 45 deg/sec for smooth roaming)
                max_yaw_rate = float(cfg.get("max_yaw_rate_deg_per_sec", 45.0))
                
                # Generate smooth yaw transition from context.yaw to target_yaw
                yaws = _smooth_yaw_transition(
                    start_yaw=context.yaw,
                    target_yaw=target_yaw,
                    num_steps=len(path_points),
                    max_yaw_rate_deg_per_sec=max_yaw_rate,
                    fps=context.fps,
                )
