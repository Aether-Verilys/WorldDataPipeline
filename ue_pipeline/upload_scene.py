...
使用方法:
    ...
    # 直接指定场景名
    python app.py upload_scene --scene LevelPrototyping
    
    # 模拟运行
    python app.py upload_scene --scene LevelPrototyping --dry-run
    ...
    if project_path:
        ...
        try:
            ...
            if not config_file.exists():
                config_file = Path(__file__).parent.parent / ue_config_path
            ...
            if config_file.exists():
                with open(config_file, 'r', encoding='utf-8') as f:
                    ...
                    if config_project:
                        ...
                        # 使用默认路径
                        content_dir = Path(__file__).parent.parent / 'ue_template' / 'project' / 'Content'
            else:
                content_dir = Path(__file__).parent.parent / 'ue_template' / 'project' / 'Content'
        except Exception:
            content_dir = Path(__file__).parent.parent / 'ue_template' / 'project' / 'Content'
...
def find_scene_path(scene_name: str, project_path: str = None) -> Path:
    ...
    # 优先级4: 默认模板路径
    default_project = Path(__file__).parent.parent / 'ue_template' / 'project'
...
def load_bos_config(config_path: str = None) -> dict:
    ...
    paths_to_try = [
        ...
        Path(__file__).parent.parent / config_path,  # 相对于脚本位置
        Path(__file__).parent / 'config' / 'bos.json',  # 相对于脚本的config目录
...
示例:
  # 列出可用场景
  python ue_pipeline/upload_scene.py --list
  
  # 交互式输入场景名（推荐）
  python ue_pipeline/upload_scene.py
  
  # 直接指定场景名
  python ue_pipeline/upload_scene.py --scene LevelPrototyping
  
  # 模拟运行
  python ue_pipeline/upload_scene.py --scene LevelPrototyping --dry-run
  
  # 覆盖本地路径
  python ue_pipeline/upload_scene.py --scene MyScene --local-path "F:/UE/Content/Game/MyScene"
