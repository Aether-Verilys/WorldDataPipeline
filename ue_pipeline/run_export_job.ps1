...
$WorkerExport = "$PSScriptRoot\python\worker_export.py"
...
Write-Host "UE Camera Export Job Executor" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if ($jobType -ne "export") {
        Write-Host "ERROR: Invalid job type '$jobType', expected 'export'" -ForegroundColor Red
        exit 1
    }
...
if (-not (Test-Path $WorkerExport)) {
    Write-Host "ERROR: Worker export script not found at: $WorkerExport" -ForegroundColor Red
    exit 1
}

Write-Host "Starting export job..." -ForegroundColor Green
Write-Host ""
...
# Build command
$pyCommand = "py `"$WorkerExport`" --manifest=`"$AbsManifestPath`""
...
try {
    ...
    if ($ueProcess.ExitCode -eq 0) {
        Write-Host "Export job completed successfully" -ForegroundColor Green
        exit 0
    } else {
        Write-Host "Export job failed with exit code: $($ueProcess.ExitCode)" -ForegroundColor Red
